...
def p_expressao_aditiva(p):
  ...
  if(len(p) == 4):
    p[0] = Node('expressao_adtiva', value = 'expressao_adtiva', children = [
      p[1],
      p[2],
      p[3]
    ])
  else:
    p[0] = Node('expressao_adtiva', value = 'expressao_adtiva', children = [p[1]])
...
def p_error(p):
  ...
  if p:
    print('Sintaxe Inválida do token \'' + str(p.value) + '\' em ' + str(p.lineno - (total_lines - 1)) + ':' + str(lex.f_column(p)))
  else:
...
def p_leia_error(p):
  ...
  print('\tProblemas na leituras dos argumentos na função \'LEIA\'')
  exit(1)
...
def p_escreva_error(p):
  ...
  print('\tProblemas na leituras dos argumentos na função \'ESCREVA\'')
  exit(1)
...
def p_retorna_error(p):
  ...
  print('\tProblemas na leituras dos argumentos na função \'RETORNA\'')
  exit(1)
...
def p_declaracao_variaveis_error(p):
  ...
  print('\tDeclaração de Variável Inválida')
  exit(1)
...
def p_atribuicao_error(p):
  ...
  print('\tAtribuição de Variável Inválida')
...
def p_lista_declaracoes_error(p):
  ...
  print('\tProblemas com Lista de Variáveis')
  exit(1)
...
def p_acao_error(p):
  ...
  print('\tErro com ação de Funções')
  exit(1)
...
def p_cabecalho_error(p):
  ...
  print('\tDeclaração de Cabeçalho de Função Inválida')
  exit(1)
...
def p_lista_parametros_error(p):
  ...
  print('\tProblemas com a Lista de Parâmetros')
  exit(1)
...
def p_expressao_simples_error(p):
  ...
  print('\tErro na Expressão Simples')
  exit(1)
